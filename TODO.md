...
保持“先汉化，再逐步改善”，不重新初始化。项目 0.1.2，底座固定 Pi 0.85.0；JavaScript ESM + npm 发布包 + 构建期显示补丁，不是扩展或完整源码分叉。

### 本次接续：Telegram 通知漏发

- [x] 只检查设置状态（未输出密钥）：当前进程无 Telegram 环境变量，但用户 `telegram.env` 存在且含两个配置字段；旧启动器不读取该文件，扩展因此跳过发送。
- [x] 新增 `scripts/telegram-config.mjs`，启动器直接读取当前用户配置目录下 `telegram.env`；仅导入两项字段，环境变量优先，不执行 shell，不读取其他用户配置；关闭通知时跳过读取。
- [x] 新增配置读取、启动器子进程传递和真实扩展事件 + 模拟网络回归测试，更新使用说明。纳入待发布 `0.1.2`。
- [x] 修复通知后重新进行完整临时安装、构建、检查，全部 25 项测试通过（通知请求为模拟网络，不调用模型 / Telegram）。
- [ ] 真实 Telegram 收件及重启后的自动通知尚未验证；本次未使用真实密钥发送测试消息，也未更新当前运行的安装副本。

### 本次接续：向 GitHub 发布版本号修复

- [x] 按用户要求准备修复版 `0.1.2`，同步 `package.json` / 锁文件及发布验证说明。
- [x] 本地发布包在独立临时目录安装、完整构建、`npm run check` 和全部 20 项测试通过；未重建当前工作区或正在运行的安装目录，未使用真实配置或密钥。
- [x] 提交 `257e129` 已推送 GitHub `main` 和 `v0.1.2` tag；创建 `v0.1.2` Release，上传包含两项修复的 `jianpai-latest.tar.gz` 并设为 latest。
- [x] 用公开安装脚本及 latest 下载 URL 隔离安装通过：中文和英文命令均输出 `0.1.2`，安装后的版本补丁及 Telegram 配置加载器与源码一致。
- [x] GitHub Actions `34043037819` 通过：Linux 干净安装、构建、检查及测试成功。
- [ ] 用户真实安装仍为旧版；需用户升级并退出重启，之后确认实际 Telegram 收件。
- [ ] 新构建启动标题尚未做终端交互体验验证；标题表达式自动测试已通过。

### 本次：修正启动版本号

- [x] 修正 `locales/patches.mjs`：启动标题和 `--version` 从简派 `package.json` 获取版本（修复时 `0.1.1`，发布版 `0.1.2`），保留 Pi 内部 `VERSION` 与包版本不变。
- [x] 新增 `test/version-display.test.mjs`，内存编译、上游指纹和标题表达式执行验证通过；更新启动器版本断言和 README。
- [x] 执行 `npm test`：20 项中 19 项通过，1 项因旧构建仍输出 `0.85.0` 而失败；未重建运行目录。
- [ ] `npm run check` 提示 `dist/main.js` 构建过期。退出后在普通终端执行 `npm run build && npm run check && npm test`，并验证新启动标题及 `--version`。
- [ ] 已安装的 `~/.jianpai/versions/` 副本尚未更新；本次不发布、不修改全局安装。新发布包安装后才能在全局命令中生效。
